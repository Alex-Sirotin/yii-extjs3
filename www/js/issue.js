...
//	Events                                                                    //
//----------------------------------------------------------------------------//		
		// Выход из поддержки, переход в корень сайта
		onExit: function(){
			location.href='/';
		},
//----------------------------------------------------------------------------//				
...
//----------------------------------------------------------------------------//				
		// Выход из формы создание темы
		onOpenIssue: function(){
			var vp = Ext.getCmp('supportViewport');
			// Тема долдны быть выбрана в onIssueSelect
			...
			if (vp.issue && !vp.issue.get('is_closed')){				
				...
				if (vp.issue.get('is_closed')){
					Ext.Msg.alert('Темы', 'Тема закрыта!');
				}				
			}
		},			
//----------------------------------------------------------------------------//				
		...
		onAddMessage: function(obj, e){
			...
			if (issue && !issue.get('is_closed') && 
				...
				if (newMessage.valueOf() != ''){
					...
					Ext.Msg.alert('Сообщения', 'Напишите сообщение!');
				}
			} else {
				// Алерты с ошибками
				if (!issue){
					Ext.Msg.alert('Сообщения', 'Выберите тему!');
				}
				
				if (issue.get('is_closed')){
					Ext.Msg.alert('Сообщения', 'Тема закрыта!');
				}				
				
				if (issue.get('status_id') == 1){
					Ext.Msg.alert('Сообщения', 'Тема не открыта!');
				}				
			}
		},
//----------------------------------------------------------------------------//		
